...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text and not update.message.text.startswith('/start'):
        # Welcome them first
        await update.message.reply_text(f"{EMERGENCY_MESSAGE}\n\n{WELCOME_MESSAGE}")
        ...
        if any(keyword in chief_complaint.lower() for keyword in EMERGENCY_KEYWORDS):
            await update.message.reply_text(EMERGENCY_DETECTED_MESSAGE)
            return ConversationHandler.END
        
        context.user_data['patient_data']['chief_complaint'] = chief_complaint
        await update.message.reply_text(ASK_DURATION_MESSAGE)
        return ASK_DURATION
...
async def handle_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ASK_SEVERITY_MESSAGE)
    return ASK_SEVERITY
...
async def handle_severity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ASK_ASSOCIATED_SYMPTOMS_MESSAGE)
...
async def handle_associated_symptoms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ASK_AGE_MESSAGE)
    return ASK_AGE
...
async def handle_age(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ASK_LOCATION_MESSAGE)
    return ASK_LOCATION
...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ASK_CHRONIC_CONDITIONS_MESSAGE)
...
async def handle_chronic_conditions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ASK_CURRENT_MEDICATIONS_MESSAGE)
...
async def handle_medications_and_triage(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(ANALYZING_MESSAGE)
    ...
    if triage_info.get('is_emergency', False):
        await update.message.reply_text(EMERGENCY_WARNING_MESSAGE)
    ...
    matches = doctor_matcher.find_doctors(
        ...
        specialty=specialty,
        online_only=False,
        limit=MAX_DOCTOR_RESULTS
    )
    
    if not matches:
        await update.message.reply_text(
            NO_DOCTORS_FOUND_MESSAGE.format(specialty=specialty) + f"\n\n{GENERAL_DISCLAIMER}"
        )
        return ConversationHandler.END
    
    # Send results
    response = DOCTORS_INTRO_MESSAGE.format(specialty=specialty, count=len(matches))
